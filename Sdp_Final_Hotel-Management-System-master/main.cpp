...
class Hotel
{
	...
	public:
		...
		{
			if(!instanceHotel)
			instanceHotel=new Hotel("TAJ","MUMBAI");
	 ...
	 void displayMenu()
	 {
		...
		void askFeedback()
		{
			...
			cin>>cname;
			feedback<<cname<<"\t\t\t";
			cout<<" Thanks for your time! \n How likely are you to recommend Hotel TAJ to a Friend or Colleague? \n Rate on a scale of 1-10\n";
			cin>>f;
			feedback<<"Feedback\t: ";
			feedback<<f<<"\n";
			feedback.close();
...
cout<<"\n";
	...
	string dname;
	Dish d2;
	Hotel *taj=taj->getHotel();

	Dish d[8]={
		Dish("Chocolate_icecream",140,"Desert"),
	...
	level2:	cout<<"Enter \n\t1. Accommodation\n\t2. Restaurant\n\t3. Exit\n";
		...
		if(ch==1)
		{
		 taj->customer[i]=new RoomCustomer;
		 taj->generateID(taj->customer[i]);
		 cout<<"Enter Your details\n";
		 taj->customer[i]->setData();
		level1:	cout<<"Enter \n\t\t1. To Display Rooms \n\t\t2. To Book a Room \n\t\t3. To Vacate Room \n\t\t4. To Get Invoice  \n\t\t5. Not Satisfied? \n\t\t6. Cancel Booking \n\t\t7. Give Feedback \n\t\t8. Back\n\n";
			cin>>ch1;
			switch(ch1)
			{
				case 1: taj->displayAvailble();
				goto level1;
				case 2:
				if(taj->customer[i]->status)
				{
				cout<<"\n\n-----------------------------------------------------------------------------------------\n\n";
				...
				{
				taj->customer[i]->status=1;
				taj->displayAvailble();
				cout<<"Enter Room No\n";
				cin>>rno;
				taj->bookRoom(rno);
				r2=taj->getRoom(rno);
				taj->customer[i]->allocateRoom(r2);
				}
				...
				goto level1;
				case 4: if(!taj->customer[i]->status)
			{
				cout<<"\n\n------------------------------------------------------------------------------------------\n\n";
				...
				{
				cout<<"\n\n-----------------------------------------------------------------\n\n";
				taj->customer[i]->printCustomer();
				taj->customer[i]->viewTotalBill();
					cout<<"\n\n-----------------------------------------------------------------\n\n";
				goto level1;}
				case 5:
				if(!taj->customer[i]->status)
			...
			cout<<"\n\n--------------------------------------------------------------------------------------------\n\n";
				...
				else
				{
					...
					e->performDuty();
				//taj->employee->performDuty()
			}
					goto level1;
				case 6: 	if(!taj->customer[i]->status)
			 {
				 	cout<<"\n\n-----------------------------------------------------------------------------------\n\n";
				...
				goto level1;
				case 7: if(!taj->customer[i]->status)
			 {
				 	cout<<"\n\n-----------------------------------------------------------------------------------\n\n";
		...
		{
	 	taj->customer[i]=new RestaurantCustomer;
		 taj->generateID(taj->customer[i]);
		 cout<<"Enter Your details\n";
		 taj->customer[i]->setData();
	level3:	cout<<"Enter \n\t\t1. To display Menu \n\t\t2. To order a Dish \n\t\t3. To Get Invoice \n\t\t4. To Cancel Order\n\t\t5. Give FeedBack \n\t\t6. Go back\n\n";
		...
		cin>>dname;
			 ...
			 d2=taj->restuarant.getDish(dname);

			taj->customer[i]->allocateDish(d2);
			...
			case 5:	if(!taj->customer[i]->status)
			 {
				 	cout<<"\n\n-----------------------------------------------------------------------------------\n\n";
				throw Exception(2,"Cannot give feedback unless you order a Dish\n");
				}
